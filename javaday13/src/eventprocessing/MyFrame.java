...
public class MyFrame extends JFrame {
	...
	public MyFrame() {
		...
		//JOptionPane.showMessageDialog(this, "메시지", "제목", JOptionPane.INFORMATION_MESSAGE);
		...
		*/
		...
		// 이미지를 가지고 아이콘을 생성
		// 이미지 파일 크기 그대로 생성됩니다.
		//icon = new ImageIcon("C:\\Users\\admin\\Documents\\image\\0.png");
						...
						public void run() {
							try {
								...
								while (true) {
									...
									Thread.sleep(100);
									// 아이콘 생성
									//icon = new ImageIcon("C:\\Users\\admin\\Documents\\image\\" + (i % 10) + ".png");
		...
		//테이블에 출력할 데이터 배열
		String [] [] data = {{"박문석", "01041553232", "서울시 양천구 목동"},
				{"제시카", "01036227569", "뉴질랜드 크라이스쳐치"}};
		...
		//Anonymous Class 안에서 this는 자기자신입니다.
		ActionListener itemListener = new ActionListener() {
			@Override
			public void actionPerformed(ActionEvent e) {
				//내부 클래스에서 외부 클래스의 인스턴스를 호출할 때는
				//외부클래스이름.this 로 합니다.
				//안드로이드에서는 이벤트 처리를 많이하고 메모리 절약을 위해서 anonymous를 많이
				//사용하기 때문에 외부클래스의 인스턴스를 호출해야 하는 상황이 많아서
				//이 문법을 기억해야 합니다.
		...
		/*
		 * int i = 0; while(true) { try { Thread.sleep(50); icon = new
		 * ImageIcon("C:\\Users\\admin\\Documents\\image\\" + (i%10) +".png");
		 * lbl.setIcon(icon); i = i + 1; }catch(Exception e) {} }
		 */
